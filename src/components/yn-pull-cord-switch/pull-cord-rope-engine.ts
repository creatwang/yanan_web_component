...
export class PullCordRopeEngine {
  ...
  private beginSnap(now: number) {
    ...
    this.snap = {
      ...
      cardW,
      cardH,
      ceilingW: t.num("--yn-pull-cord-switch-ceiling-width"),
  ...
  private relayout() {
    ...
    const layout = resolveAnchor(this.h, ceilingW, this.theme.num("--yn-pull-cord-switch-anchor-y"));
  ...
  private cardAnchor() {
    return this.options.getCardAnchor?.() ?? "center";
  }

  private cardLayout(end: number) {
    const { card, cardOff } = this.snap;
    const cx = this.px[end];
    const ropeY = this.py[end];
    if (this.cardAnchor() === "top") {
  ...
  private draw() {
    ...
    const ctx = this.ctx;
    const { seg, cardOff, ceilingW, card } = this.snap;
    const { ceilingH, anchorR, ceilingTop } = resolveAnchor(
      this.h,
      ceilingW,
      this.theme.num("--yn-pull-cord-switch-anchor-y")
    );
    ...
    const end = seg;
    const ropeEndY = this.py[end];
    const { attachY } = this.cardLayout(end);
    ctx.strokeStyle = "rgba(0,0,0,0.4)";
    ctx.lineWidth = 2;
    ctx.beginPath();
    ctx.moveTo(this.px[end], ropeEndY);
  ...
  private tick = () => {
    ...
    const active = this.dragging || energy > IDLE_EPS || this.idleFrames < 4;
    if (active) this.draw();
