...
/** Production: cross-border checkout excludes mainland China. */
export const EXCLUDE_COUNTRY_CODES = new Set(["CN"]);
...
const CHINA_HINTS = ["beijing", "peking", "shanghai", "guangzhou", "shenzhen", "china"];

let countriesCache: ICountry[] | null = null;

const norm = (v: string) => v.trim().toLowerCase();

export function isChinaQuery(query: string) {
  const q = norm(query);
...
export async function loadCountries(force = false) {
  ...
  countriesCache = all.filter((c) => !EXCLUDE_COUNTRY_CODES.has(c.iso2));
  return countriesCache;
}
...
async function citiesInCountry(
  ...
  const stateHits = states.filter((s) => s.name.toLowerCase().includes(q));
...
export async function searchDr5hnRegions(query: string, signal: AbortSignal) {
  const q = norm(query);
  ...
  for (const country of pickCountries(query, countries)) {
    ...
    for (const item of await citiesInCountry(country, query, signal)) {
      push(item);
    }
  }
