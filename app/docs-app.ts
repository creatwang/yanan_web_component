...
export class YnDocsApp extends LitElement {
  ...
  private renderStyleDependencies(page: ResolvedComponentPage) {
    const hasCssVars = page.cssVars.length > 0;
    const usesTailwindDemo = [
      "yn-button",
    ...
    return html`
      <h3>${this.locale === "zh-CN" ? "样式依赖" : "Style dependencies"}</h3>
      <ul>
        <li>
          ${this.locale === "zh-CN"
            ? "组件自身样式封装在 Shadow DOM 中，业务使用时不依赖 Tailwind CSS。"
            : "Component styles are encapsulated in Shadow DOM and do not require Tailwind CSS in consumer apps."}
        </li>
        ${hasCssVars
          ? html`
              <li>
                ${this.locale === "zh-CN"
                  ? "当前模板可按需覆写这些公开 CSS 变量："
                  : "The template can override these public CSS variables when needed:"}
                <code>${page.cssVars.map((v) => v.name).join(", ")}</code>
              </li>
            `
          : nothing}
        ${usesTailwindDemo
          ? html`
              <li>
                ${this.locale === "zh-CN"
                  ? "文档站 live demo 中出现的 `yn-*` class 是 Tailwind 展示容器样式；复制到业务项目时可换成普通 CSS。"
                  : "`yn-*` classes in live demos are Tailwind utility classes for demo containers only; replace them with regular CSS in your app if needed."}
              </li>
            `
          : nothing}
        ${page.id === "yn-search"
          ...
          <header class="docs-header">
            ...
            <div class="docs-header__links">
              ...
              <a
                ...
                href="https://github.com/creatwang/yanan_web_component"
                target="_blank"
                rel="noopener noreferrer"
                aria-label="GitHub"
                title="GitHub"
